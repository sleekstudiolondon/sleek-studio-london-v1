...
export default function DemoSiteClient({ site }: DemoSiteClientProps) {
  if (site.slug === "maison-form") return <MaisonDemoSite site={site} />;
  if (site.slug === "studio-alter") return <AlterDemoSite site={site} />;
  return <JohnDemoSite site={site} />;
}

function useDemoNavigation(site: CaseStudy) {
  const tabs = useMemo(
...
function JohnDemoSite({ site }: { site: CaseStudy }) {
  ...
  return (
    <article className={`demo-site demo-site-tabbed ${site.palette}`} id="home">
      ...
      <div className="demo-tab-shell">
        ...
        {activeTab === "portfolio" ? <ProjectPanel site={site} activeTab={activeTab} projects={getProjectsForTab(site, activeTab)} /> : null}
...
function StudioAlterDemoSite({ site }: { site: CaseStudy }) {
  const { tabs, activeTab, selectTab } = useDemoNavigation(site);

  return (
    <article className="demo-site alter-site-reset demo-alter" id="home">
      <AlterNav site={site} tabs={tabs} activeTab={activeTab} selectTab={selectTab} />
      <main className="alter-page-shell">
        {activeTab === "home" ? <AlterHomePanel site={site} selectTab={selectTab} /> : null}
        {activeTab === "studio" ? <AlterStudioSystem site={site} /> : null}
        {activeTab === "projects" ? <AlterProjectArchive site={site} projects={site.projects} /> : null}
        {activeTab === "services" ? <AlterServicesPanel site={site} /> : null}
        {activeTab === "journal" ? <AlterJournalIndex site={site} /> : null}
        {activeTab === "contact" ? <AlterContactBrief site={site} /> : null}
      </main>
    </article>
  );
}

function AlterDemoSite(props: { site: CaseStudy }) {
  return <StudioAlterDemoSite {...props} />;
}

function MaisonDemoSite({ site }: { site: CaseStudy }) {
  const { tabs, activeTab, selectTab } = useDemoNavigation(site);

  return (
    <article className="demo-site maison-site-reset demo-maison" id="home">
      <MaisonNav site={site} tabs={tabs} activeTab={activeTab} selectTab={selectTab} />
      <main className="maison-page-shell">
        {activeTab === "home" ? <MaisonHomePanel site={site} selectTab={selectTab} /> : null}
        {activeTab === "studio" ? <MaisonStudioPanel site={site} /> : null}
        {activeTab === "residences" ? <MaisonPortfolioPanel site={site} activeTab={activeTab} projects={getProjectsForTab(site, activeTab)} /> : null}
        {activeTab === "hospitality" ? <MaisonHospitalityPanel site={site} projects={getProjectsForTab(site, activeTab)} /> : null}
        {activeTab === "atelier" ? <MaisonAtelierPanel site={site} /> : null}
        {activeTab === "press" ? <MaisonPressPanel site={site} /> : null}
        {activeTab === "contact" ? <MaisonContactPanel site={site} /> : null}
      </main>
    </article>
  );
}
      ...
      <div className="alter-archive-list">
        {rest.map((project, index) => (
          <article key={project.title}>
            ...
            <div className="alter-archive-thumb">
              <Image src={project.image} alt={`${project.title} archive thumbnail`} width={520} height={360} className="demo-project-image" sizes="(max-width: 900px) 100vw, 12vw" loading="lazy" />
            </div>
            <h3>{project.title}</h3>
...
function AlterServicesPanel({ site }: { site: CaseStudy }) {
  return (
    <section className="demo-tab-panel alter-services-page" id="services" aria-labelledby="alter-services-title">
      <div className="alter-archive-head">
        <p className="demo-kicker">Services</p>
        <h2 id="alter-services-title">Interior architecture, residential design, and objects as one system.</h2>
      </div>
      <div className="alter-services-matrix">
      ...
      <div className="alter-process-ledger">
...
function AlterJournalIndex({ site }: { site: CaseStudy }) {
  return (
    <section className="demo-tab-panel alter-journal-index" id="journal" aria-labelledby="alter-journal-title">
      <div className="alter-archive-head">
        <p className="demo-kicker">Journal</p>
        <h2 id="alter-journal-title">Material notes, archive fragments, and studio letters.</h2>
      </div>
      <div className="alter-article-ledger">
        {site.journal?.map((item, index) => (
...
function MaisonHospitalityPanel({ site, projects }: { site: CaseStudy; projects: DemoProject[] }) {
  const [feature, ...plates] = projects;
  return (
    <section className="demo-tab-panel maison-hospitality-panel" id="hospitality" aria-labelledby="maison-hospitality-title">
      <div className="maison-hospitality-story">
        <div>
          <p className="demo-kicker">Hospitality</p>
          <h2 id="maison-hospitality-title">Guest spaces with theatre, privacy, and operational poise.</h2>
          <p>Hospitality work is treated as atmosphere in motion: arrival, pause, service, retreat, and memory are composed with the same discretion as a private residence.</p>
        </div>
        {feature ? (
          <div className="maison-hospitality-image">
            <Image src={feature.image} alt={`${feature.title} by ${site.title}`} width={1800} height={1200} className="demo-project-image" sizes="(max-width: 900px) 100vw, 54vw" loading="eager" />
          </div>
        ) : null}
      </div>
      <div className="maison-hospitality-plates">
        {[feature, ...plates].filter(Boolean).map((project, index) => (
          <article key={project.title}>
            <span>{String(index + 1).padStart(2, "0")} · {project.location}</span>
            <h3>{project.title}</h3>
            <p>{project.description}</p>
          </article>
        ))}
...
function MaisonStudioPanel({ site }: { site: CaseStudy }) {
  return (
    <section className="demo-tab-panel maison-studio-panel" id="studio" aria-labelledby="maison-studio-title">
      <div className="maison-studio-opening">
        <p className="demo-kicker">Studio</p>
        <h2 id="maison-studio-title">An established practice for private homes, hotels, and international commissions.</h2>
        <p>{site.philosophy.copy}</p>
      </div>
      <div className="maison-principals-row">
      ...
      <div className="maison-international-block">
        <div>
          <p className="demo-kicker">International commissions</p>
          <h3>Homes and hospitality spaces coordinated with discretion across cities, estates, and private offices.</h3>
        </div>
        <div className="maison-international-list">
          <span>London townhouses</span>
          <span>Paris apartments</span>
          <span>Geneva villas</span>
          <span>Hospitality suites</span>
        </div>
      </div>
      <div className="maison-discretion-timeline">
        {site.process?.map((step, index) => (
          <article key={step.title}>
            <span>{String(index + 1).padStart(2, "0")}</span>
            <h3>{step.title}</h3>
