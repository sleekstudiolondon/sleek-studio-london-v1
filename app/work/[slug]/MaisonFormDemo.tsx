...
const maisonStats = ["100+ private commissions", "25+ countries", "63 years of atelier heritage", "40+ craft partners"];
...
const maisonTimeline = [
  ...
  ["Today", "Atelier-led interiors, procurement, and installation stewardship across Europe, the Middle East, and North America."],
];
  ...
  return (
    <section className="maison2-page maison3-home" aria-labelledby="maison3-home-title">
      <div className="maison3-text-hero">
        ...
        <h1 id="maison3-home-title">Private interiors with provenance and restraint.</h1>
        <p>{site.hero.copy}</p>
      ...
      <div className="maison3-stat-strip" aria-label="Maison Form studio credentials">{maisonStats.map((item) => <span key={item}>{item}</span>)}</div>
...
function MaisonStudio({ site, selectTab }: { site: CaseStudy; selectTab: (key: string) => void }) {
  return (
    <section className="maison2-page maison2-studio maison3-about" id="studio" aria-labelledby="maison2-studio-title">
      <div className="maison2-studio-opening"><p className="maison2-kicker">About Us</p><h2 id="maison2-studio-title">A Paris-born private interiors atelier with 63 years of quiet commissions.</h2><p>Founded in 1963, Maison Form is imagined as an established private-client studio: discreet, atelier-led, and fluent in residences, hospitality projects, procurement, and exacting installation.</p></div>
      <div className="maison3-stat-strip maison3-about-stats" aria-label="Maison Form heritage statistics">{["100+ completed commissions", "25+ countries", "3 generations of craft network", "Private residences, hotels, and atelier procurement"].map((item) => <span key={item}>{item}</span>)}</div>
      <div className="maison2-principals">{site.team?.map((person) => <article key={person.title}><span>Leadership</span><h3>{person.title}</h3><p>{person.copy}</p></article>)}</div>
      <div className="maison3-timeline">{maisonTimeline.map(([year, copy]) => <article key={year}><span>{year}</span><p>{copy}</p></article>)}</div>
      <div className="maison3-principles">{maisonPrinciples.map(([title, copy]) => <article key={title}><h3>{title}</h3><p>{copy}</p></article>)}</div>
      <div className="maison2-international"><h3>International commissions across Europe, the Middle East, and North America, coordinated through discretion, procurement, and exacting site presence.</h3><div>{["Paris atelier", "London townhouses", "Geneva villas", "Hospitality suites"].map((item) => <span key={item}>{item}</span>)}</div></div>
      ...
      <div className="maison3-closing-rule"><p>The practice is deliberately private: senior attention, quiet coordination, and careful handover are treated as part of the design work.</p><button type="button" onClick={() => selectTab("portfolio")}>View portfolio</button></div>
    </section>
  );
}
...
function MaisonPress({ site }: { site: CaseStudy }) {
  ...
  return (
    <section className="maison2-page maison2-press maison3-press" id="press" aria-labelledby="maison2-press-title">
      <div className="maison2-section-head"><p className="maison2-kicker">Press & recognition</p><h2 id="maison2-press-title">Featured in international design journals and private-client circles.</h2><p>Recognition is treated as a quiet archive: publication notes, private referrals, and completed rooms that carry their own authority.</p></div>
      <div className="maison3-press-stats">{["International design journals", "Private residences", "Hospitality interiors", "Atelier procurement"].map((item) => <span key={item}>{item}</span>)}</div>
      {featured ? <article className="maison3-featured-press"><span>Featured story</span><h3>{featured.source}</h3><p>{featured.quote}</p></article> : null}
      ...
      <div className="maison3-awards">{["Recognised for private residences", "Hospitality atmosphere and service", "Craft network and procurement", "International commissions across Europe, the Middle East, and North America"].map((item) => <p key={item}>{item}</p>)}</div>
