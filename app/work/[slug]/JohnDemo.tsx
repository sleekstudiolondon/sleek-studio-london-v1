...
function JohnNav({ site, tabs, activeTab, selectTab }: { site: CaseStudy; tabs: DemoTab[]; activeTab: string; selectTab: (key: string) => void }) {
  return (
    <header className="john-demo-nav">
      ...
      <Link href="/work" className="john-demo-return">Back to Sleek Studio</Link>
    </header>
  );
}
...
function JohnHome({ site, selectTab }: { site: CaseStudy; selectTab: (key: string) => void }) {
  ...
  return (
    <section className="john-page john-home" aria-labelledby="john-home-title">
      ...
      <div className="john-trust-strip" aria-label="John Doe working approach">
        <span>Small studio attention</span>
        <span>Residential sourcing</span>
        <span>Measured, personal process</span>
      </div>
      {feature ? (
      ...
      <div className="john-closing-prompt">
        <p>Begin with a room that feels almost right, or a house that needs a calmer hand.</p>
        <button type="button" onClick={() => selectTab("contact")}>Tell John about the home</button>
      </div>
    </section>
  );
}
...
function JohnStudio({ site }: { site: CaseStudy }) {
  return (
    <section className="john-page john-studio" id="studio" aria-labelledby="john-studio-title">
      ...
      <div className="john-process-timeline">
        {[
          ["Listen", "A conversation about the house, the people in it, and the rooms that do not yet feel settled."],
          ["Edit", "A quiet pass through light, storage, circulation, and materials before anything decorative is added."],
          ["Layer", "Furniture, textiles, antiques, and objects are brought together slowly so the result feels collected."],
        ].map(([title, copy], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><h3>{title}</h3><p>{copy}</p></article>)}
      </div>
      <div className="john-soft-cta"><p>For clients who want the home to feel quieter, warmer, and more considered before anything is bought.</p></div>
    </section>
  );
}
...
function JohnPortfolio({ site }: { site: CaseStudy }) {
  ...
  return (
    <section className="john-page john-portfolio" id="portfolio" aria-labelledby="john-portfolio-title">
      ...
      <div className="john-soft-cta"><p>Portfolio conversations usually begin with one room, then widen into the way the whole home should feel.</p></div>
    </section>
  );
}
...
function JohnRoomStudy({ site, project }: { site: CaseStudy; project: DemoProject }) {
  return (
    <article className="john-room-study">
      ...
      <dl>
        <div><dt>Mood</dt><dd>{project.title.includes("Reading") ? "Quiet and enclosed" : "Warm and lived-in"}</dd></div>
        <div><dt>Materials</dt><dd>Linen, oak, stone, and collected objects</dd></div>
      </dl>
    </article>
  );
}
...
function JohnServices({ site }: { site: CaseStudy }) {
  return (
    <section className="john-page john-services" id="services" aria-labelledby="john-services-title">
      ...
      <div className="john-service-questions">
        <p className="john-kicker">Useful when</p>
        {["A room has good bones but no atmosphere.", "A renovation needs calmer material direction.", "A home needs final layers, sourcing, and edit."].map((item) => <span key={item}>{item}</span>)}
      </div>
      <div className="john-faq-strip">
        {["Can we start with one room? Yes — a focused room study often clarifies the rest of the home.", "Do you source furniture and art? Yes — sourcing is handled as part of the atmosphere, not as a separate shopping list.", "Can you work with existing pieces? Preferably — the best homes keep what already has meaning."].map((item) => <p key={item}>{item}</p>)}
      </div>
    </section>
  );
}
