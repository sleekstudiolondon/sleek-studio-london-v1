  ...
  return (
    <section className="alter2-page alter2-services" id="services" aria-labelledby="alter2-services-title">
      ...
      <div className="alter2-commission-flow">
        {["Survey", "Material argument", "Spatial edit", "Installation direction"].map((step, index) => <article key={step}><span>{String(index + 1).padStart(2, "0")}</span><h3>{step}</h3><p>{index === 0 ? "Existing rhythm, light, and constraints are mapped before style is discussed." : index === 1 ? "Samples are tested as a system so every surface has a reason." : index === 2 ? "Plans are reduced until the strongest lines remain." : "Final objects, lighting, and site direction close the loop."}</p></article>)}
      </div>
    </section>
  );
}
  ...
  return (
    <section className="alter2-page alter2-journal" id="journal" aria-labelledby="alter2-journal-title">
      ...
      <div className="alter2-article-index">
        ...
        {["Surveying awkward corners", "Why objects need negative space", "A note on steel in domestic rooms"].map((title, index) => <article key={title}><span>{String(index + 4).padStart(2, "0")}</span><p>Studio note</p><h3>{title}</h3><em>Short observations from site, sample boards, and the studio archive.</em></article>)}
      </div>
    </section>
  );
}
