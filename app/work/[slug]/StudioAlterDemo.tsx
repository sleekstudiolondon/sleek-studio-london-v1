...
function AlterNav({ site, tabs, activeTab, selectTab }: { site: CaseStudy; tabs: DemoTab[]; activeTab: string; selectTab: (key: string) => void }) {
  return (
    <header className="alter2-nav">
      ...
      <Link href="/work" className="alter2-return">Back to Sleek Studio</Link>
    </header>
  );
}
...
function AlterHome({ site, selectTab }: { site: CaseStudy; selectTab: (key: string) => void }) {
  ...
  return (
    <section className="alter2-page alter2-home" aria-labelledby="alter2-home-title">
      ...
      <div className="alter2-concerns">
        {[
          ["Current concern", "How to make contemporary homes feel precise without becoming cold."],
          ["Material test", "Honed stone, clay plaster, olive lacquer, dark metal, and soft textile are balanced as one family."],
          ["Spatial question", "Which line can be removed so the room becomes calmer and more architectural?"],
        ].map(([label, copy]) => <article key={label}><span>{label}</span><p>{copy}</p></article>)}
      </div>
      <div className="alter2-closing-cta"><p>Studio Alter works best when the brief asks for clarity, edited material, and a sharper way of living.</p><button type="button" onClick={() => selectTab("contact")}>Discuss a commission</button></div>
    </section>
  );
}
  ...
  return (
    <section className="alter2-page alter2-studio" id="studio" aria-labelledby="alter2-studio-title">
      <div className="alter2-section-head">
        <p className="alter2-kicker">Studio</p>
      ...
      <div className="alter2-method-team">
        <div><span>Studio method</span><h3>Every scheme is reviewed through plan, material, and object.</h3></div>
        <p>A small team keeps the work close: site observations are translated into material samples, joinery studies, and a final edit of furniture and art. The result is deliberately contemporary but never detached from daily life.</p>
      </div>
      <div className="alter2-closing-cta"><p>The studio keeps each project legible from survey through installation, so decisions remain precise rather than decorative.</p></div>
    </section>
  );
}
...
function AlterProjects({ site }: { site: CaseStudy }) {
  ...
  return (
    <section className="alter2-page alter2-projects" id="projects" aria-labelledby="alter2-projects-title">
      <div className="alter2-section-head">
        <p className="alter2-kicker">Portfolio</p>
      ...
      <div className="alter2-project-narrative">
        <p className="alter2-kicker">Archive notes</p>
        <div>
          <h3>Projects are catalogued by the spatial move, not by decoration.</h3>
          <p>Each archive entry records what changed in the architecture: a threshold clarified, a material family tightened, a view opened, or storage absorbed into the room.</p>
        </div>
      </div>
      <div className="alter2-thumbnail-notes">{site.projects.map((project) => <p key={project.title}><strong>{project.title}</strong><span>{project.location} / {project.category}</span></p>)}</div>
    </section>
  );
}
...
function AlterServices({ site }: { site: CaseStudy }) {
  ...
  return (
    <section className="alter2-page alter2-services" id="services" aria-labelledby="alter2-services-title">
      <div className="alter2-section-head"><p className="alter2-kicker">Services</p><h2 id="alter2-services-title">Capabilities arranged as a design system.</h2></div>
      ...
      <div className="alter2-commission-flow">
        {["Survey", "Material argument", "Spatial edit", "Installation direction"].map((step, index) => <article key={step}><span>{step}</span><h3>{step}</h3><p>{index === 0 ? "Existing rhythm, light, and constraints are mapped before style is discussed." : index === 1 ? "Samples are tested as a system so every surface has a reason." : index === 2 ? "Plans are reduced until the strongest lines remain." : "Final objects, lighting, and site direction close the loop."}</p></article>)}
      </div>
      <div className="alter2-closing-cta"><p>Capabilities stay deliberately connected: architecture, materials, furniture, and installation are reviewed as one system.</p></div>
    </section>
  );
}
...
function AlterJournal({ site }: { site: CaseStudy }) {
  ...
  return (
    <section className="alter2-page alter2-journal" id="journal" aria-labelledby="alter2-journal-title">
      <div className="alter2-section-head"><p className="alter2-kicker">Journal</p><h2 id="alter2-journal-title">Material notes and studio letters.</h2></div>
      ...
      <div className="alter2-article-index">
        {rest.map((item) => <article key={item.title}><span>{item.meta}</span><p>{item.meta}</p><h3>{item.title}</h3><em>{item.copy}</em></article>)}
        {["Surveying awkward corners", "Why objects need negative space", "A note on steel in domestic rooms"].map((title) => <article key={title}><span>Studio note</span><p>Archive observation</p><h3>{title}</h3><em>Short observations from site, sample boards, and the studio archive.</em></article>)}
      </div>
      <div className="alter2-closing-cta"><p>Journal entries are used inside the studio as a working archive: a record of decisions that make rooms clearer.</p></div>
    </section>
  );
}
  ...
  return (
    <section className="alter2-page alter2-contact" id="contact" aria-labelledby="alter2-contact-title">
      ...
      <form className="alter2-brief" onSubmit={(event) => { event.preventDefault(); setSubmitted(true); }}>
        <div className="alter2-field-group"><span>Identity</span><label><small>Name</small><input name="name" type="text" autoComplete="name" required /></label><label><small>Email</small><input name="email" type="email" autoComplete="email" required /></label></div>
        <div className="alter2-field-group"><span>Site</span><label><small>Site / project location</small><input name="location" type="text" placeholder="Hackney townhouse, Marylebone flat, coastal site…" required /></label><label><small>Existing architecture type</small><select name="architecture" defaultValue="" required><option value="" disabled>Select one</option><option>Townhouse</option><option>Apartment</option><option>Lateral home</option><option>New-build shell</option></select></label></div>
        <div className="alter2-field-group"><span>Brief</span><label><small>Commission type</small><input name="commission" type="text" placeholder="Interior architecture, residential design, object direction…" required /></label></div>
        <div className="alter2-field-group"><span>Notes</span><label><small>Material priorities / survey notes</small><textarea name="message" rows={6} placeholder="Tell us about the architecture, constraints, materials, and what should change." required /></label></div>
...
export default function StudioAlterDemo({ site }: { site: CaseStudy }) {
  ...
  return (
    <article className="demo-site alter2-site" id="home">
